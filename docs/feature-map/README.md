...
| Review comments on diff lines | Draft line-anchored comments on a chat diff block (hover gutter **+**, drag for a range); drafts collect in a bar above the composer and ride the next normal send as a quoted block. Offered on every `ChatPage` host (main chat, embedded and popout) — split-view panes and the side chat render diffs without the drafting gutter (their composers do not drain drafts) | Chat (`/chat`) diff block → hover a line’s gutter → **+** | `components/DiffBlock.tsx`, `components/ReviewCommentBar.tsx`, `store/reviewComments.ts` (drained by `pages/ChatPage.tsx` send, wired through `pierre/PierreImpl.tsx`) | client-only (no handler; drafts live per tab in sessionStorage) | none |
